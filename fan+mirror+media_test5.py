...
cap = cv2.VideoCapture(0) #cv2.CAP_V4L 경고시 인자 추가
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        frame_flip = cv2.cvtColor(frame_flip, cv2.COLOR_RGB2BGR) # 고친 부분
        ...
        results = face_detection.process(frame_flip) #face output
        ...
        if results.detections:
            for detection in results.detections:
                ...
                # prevent the image from going off the frame
                    max_x = w - new_width
                    max_y = h - new_height
